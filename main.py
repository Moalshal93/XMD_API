...
async def extract_customers_details(session, customers_rows):
    ...
    for row in customers_rows:  
        ...
        for idx, sales_row in enumerate(sales_rows):
            ...
            itemID_rows = order_sel.xpath("//table[@class='table-section table-bordered' and contains(., 'ItemID')]//tbody//tr")
            itemID_texts = [tr.xpath(".//td[1]/text()").get() for tr in itemID_rows]
            descriptions = [tr.xpath(".//td[2]/text()").get() for tr in itemID_rows]
            items_list.extend(itemID_texts)
            desc_list.extend(descriptions)
...
# curl -X POST "https://xmd-50bg.onrender.com/newcustomer" ^
#   -d "{\"username\":\"peytonclark1997@gmail.com\",\"password\":\"Eva2022!\",\"days\":1}"
